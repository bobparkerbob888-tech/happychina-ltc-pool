...
/// POST /api/admin/reconcile-balances - Recalculate all balances from earnings - completed withdrawals
pub async fn reconcile_balances(
    ...
    // Recalculate: balance = SUM(earnings) - SUM(completed withdrawals)
    let result = sqlx::query(
        "UPDATE balances b SET amount = sub.correct_amount
         FROM (
             SELECT e.miner, e.coin, 
                    e.total_earned - COALESCE(w.total_withdrawn, 0) as correct_amount
             FROM (
                 SELECT miner, coin, SUM(amount) as total_earned
                 FROM earnings
                 GROUP BY miner, coin
             ) e
             LEFT JOIN (
                 SELECT miner, coin, SUM(amount) as total_withdrawn
                 FROM withdrawals
                 WHERE status = 'completed'
                 GROUP BY miner, coin
             ) w ON e.miner = w.miner AND e.coin = w.coin
         ) sub
         WHERE b.miner = sub.miner AND b.coin = sub.coin AND b.amount != sub.correct_amount"
    )
    .execute(db.pool())
    .await;
    ...
    match result {
        Ok(r) => {
            let fixed = r.rows_affected();
            log::info!("Balance reconciliation: {} balances corrected", fixed);
            HttpResponse::Ok().json(serde_json::json!({
                "success": true,
                "balances_corrected": fixed
            }))
        }
        Err(e) => {
